...
class Detect(nn.Module):
    ...
    def __init__(self, nc=80, ch=()):
        ...
        # self.stem = nn.ModuleList(nn.Sequential(Conv(x, x, 3),Conv(x, x, 3))for x in ch)# 后加的
        # self.stem = nn.ModuleList(nn.Sequential(Conv(x, x, 3, g = x // 16), Conv(x, x, 3, g=x//16))for x in ch)# 后加的
        # self.stem = nn.ModuleList(nn.Sequential(Conv(x, x, 1), Conv(x, x, 3))for x in ch)# 后加的
