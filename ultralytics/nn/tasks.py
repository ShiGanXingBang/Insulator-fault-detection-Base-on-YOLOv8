...
from ultralytics.nn.modules.ViT import VisionTransformer
from ultralytics.nn.modules.SimAM import SimAM
from ultralytics.nn.modules.MHSA import MHSA
from ultralytics.nn.modules.CBAM import ChannelAttention, SpatialAttention, CBAMBlock
from ultralytics.nn.modules.A2Attention import DoubleAttention
...
from ultralytics.nn.modules.ShuffleNet import ShuffleNetV2,Conv_maxpool
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in {
            ...
            args = [ch[f]]
        elif m in {VisionTransformer}:
            args = [ch[f], *args]
        elif m in {MHSA,ChannelAttention,SpatialAttention,CBAMBlock,DoubleAttention,GAM_Attention}:
            args = [ch[f], *args]     
        # elif m is mobilenetv4_small:
        #     m = m(*args)
        #     c2 = m.channel   
        elif m in [ShuffleNetV2, Conv_maxpool,AKConv]:
            c1, c2 = ch[f], args[0]
            if c2 != nc:  # if c2 not equal to number of classes (i.e. for Classify() output)
